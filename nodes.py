...
class reactor:
    ...
    def _load_model_cached(self, model_path, model_type):
        """缓存模型加载,避免重复加载"""
        if (self.cached_model is None or 
            self.cached_model_path != model_path or 
            self.cached_model_type != model_type):
            
            device = model_management.get_torch_device()
            
            if "codeformer" in model_type.lower():
                ...
                self.cached_model = codeformer_net.eval()
                
            elif ".onnx" in model_type:
                self.cached_model = set_ort_session(model_path, providers=providers)
                
            else:
                sd = comfy.utils.load_torch_file(model_path, safe_load=True)
                self.cached_model = model_loading.load_state_dict(sd).eval()
                self.cached_model.to(device)
                
            self.cached_model_path = model_path
            self.cached_model_type = model_type
            
            logger.status(f"Loaded model: {model_type}")
    ...
    def restore_face(
        ...
        if face_restore_model != "none" and not model_management.processing_interrupted():
            ...
            facerestore_model = self._load_model_cached(model_path, face_restore_model)
            
            if faceSize != FACE_SIZE or self.face_helper is None:
                self.face_helper = FaceRestoreHelper(
                    1, face_size=faceSize,
                    crop_ratio=(1, 1),
